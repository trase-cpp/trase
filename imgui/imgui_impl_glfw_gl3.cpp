...
// inputs, OpenGL/Vulkan graphics context creation, etc.) (GL3W is a helper
// library to access OpenGL functions since there is no standard header to
// access modern OpenGL functions easily. Alternatives are GLEW, Glad, etc.)

// Implemented features:
//  [X] User texture binding. Cast 'GLuint' OpenGL texture identifier as
//  void*/ImTextureID. Read the FAQ about ImTextureID in imgui.cpp. [X] Gamepad
//  navigation mapping. Enable with 'io.ConfigFlags |=
...
//  context won't fail if the function is a NULL pointer. 2018-03-20: Misc:
//  Setup io.BackendFlags ImGuiBackendFlags_HasMouseCursors and
//  ImGuiBackendFlags_HasSetMousePos flags + honor
//  ImGuiConfigFlags_NoMouseCursorChange flag. 2018-03-06: OpenGL: Added const
//  char* glsl_version parameter to ImGui_ImplGlfwGL3_Init() so user can
//  override the GLSL version e.g. "#version 150". 2018-02-23: OpenGL: Create
//  the VAO in the render function so the setup can more easily be used with
//  multiple shared GL context. 2018-02-20: Inputs: Added support for mouse
//  cursors (ImGui::GetMouseCursor() value, passed to glfwSetCursor()).
//  2018-02-20: Inputs: Renamed GLFW callbacks exposed in .h to not include GL3
//  in their name. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback
//  and exposed ImGui_ImplGlfwGL3_RenderDrawData() in the .h file so you can
//  call it yourself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which
...
//  io.WantSetMousePos flag by repositioning the mouse
//  (ImGuiConfigFlags_NavEnableSetMousePos is set). 2018-01-20: Inputs: Added
//  Horizontal Mouse Wheel support. 2018-01-18: Inputs: Added mapping for
//  ImGuiKey_Insert. 2018-01-07: OpenGL: Changed GLSL shader version from 330 to
//  150. (Also changed GL context from 3.3 to 3.2 in example's main.cpp)
//  2017-09-01: OpenGL: Save and restore current bound sampler. Save and restore
//  current polygon mode. 2017-08-25: Inputs: MousePos set to -FLT_MAX,-FLT_MAX
//  when mouse is unavailable/missing (instead of -1,-1). 2017-05-01: OpenGL:
//  Fixed save and restore of current blend function state. 2016-10-15: Misc:
...
bool ImGui_ImplGlfwGL3_CreateDeviceObjects() {
    ...
    const GLchar* vertex_shader_with_version[2] = {g_GlslVersion,
                                                   vertex_shader};
